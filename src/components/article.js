...
function Article({ article, onBtnClick, deleteArticle, loadArticle, id, dictionary }) {
  useEffect(() => {
  ...
  return (
    <div ref={setContainerRef}>
      <h3>{article.title}</h3>
      <button onClick={() => deleteArticle(article.id)}>{dictionary['delete']}</button>
      {getBody({ article })}
    </div>
  )
}
